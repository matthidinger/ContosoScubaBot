...
namespace ContosoScuba.Bot.Services
{
    public class ScubaCardService
    {
        ...
        //{
        //    var botdata = context.UserData;
        //    var userScubaData = botdata.GetValue<UserScubaData>("ScubaData");

        //    //////var weatherCard = new WeatherService().GetFakeWeatherCard(userScubaData.Destination, userScubaData.Date);
        ...
        CardTransitionInfo GetCardTransitionInfo(IDialogContext context, Activity activity)
        {
            ...
            returnValue.NextCardName = "1-Schools";
            ...
            if (!missingValue || userScubaData != null)
            {
                ...
                if (!missingValue)
                    switchStep = new Tuple<string, string>(value.First.Path.ToLower(), value.Value<string>(value.First.Path));
                else
                ...
                switch (switchStep.Item1)
                {
                    ...
                    case "mealoptions":
                        returnValue.NextCardName = "6-PersonalInfo";
                        userScubaData.MealOptions = switchStep.Item2;
                        break;
                    case "personalinfo":
                        returnValue.NextCardName = "7-Weather";
                        break;
                    default:
                        break;
                }
        ...
        Tuple<string,string> GetSwitchStepFromText(string text)
        {
            ...
            if (!string.IsNullOrEmpty(text))
            {
                ...
                else if (IsMealOption(text))
                {
                    ...
                    step = new Tuple<string, string>("personalInfo", text);
            }
            else
            {
                step = new Tuple<string, string>("date", text);
        ...
        {
            return await GetCardText(transitionInfo.NextCardName, transitionInfo.ReplaceText, transitionInfo.ReplaceWithText);
        }

        public async Task<string> GetCardText(string cardName, string replaceText = "", string replaceWith = "")
        {
            ...
            using (var f = File.OpenText(path))
            {
                ...
                return json.Replace(replaceText, replaceWith);
            }
        } 
    }
}